...
test.describe("Signup form — aware path", () => {
  test("submits and shows 'Check your inbox' with the entered email", async ({ page }) => {
    ...
    await page.locator("#keeperEmail").fill(`keeper-${uid}@example.com`);
  ...
  test("Continue requires all fields to be filled", async ({ page }) => {
    ...
    await page.locator("#firstName").fill("E2E");
    await page.locator("#lastName").fill("Maker");
    await page.locator("#email").fill(`e2e-signup-${uid}@example.com`);
...
test.describe("Signup form — private path", () => {
  ...
  test("private path requires TC fields — disabled without them", async ({ page }) => {
    ...
    await page.locator("#firstName").fill("E2E");
    await page.locator("#lastName").fill("Maker");
    await page.locator("#email").fill(`e2e-signup-priv-${uid}@example.com`);
    await page.locator("#keeperFirst").fill("E2E");
    await page.locator("#keeperLast").fill("Keeper");
    await page.locator("#keeperEmail").fill(`keeper-priv-${uid}@example.com`);
  ...
  test("private path submits when all fields including TC are filled", async ({ page }) => {
    ...
    await page.locator("#keeperLast").fill("Keeper");
    await page.locator("#keeperEmail").fill(`keeper-priv-${uid}@example.com`);
    await page.locator("#keeperRel").selectOption("Sibling");

    // Fill TC fields (assuming #tcFirst, #tcLast, #tcEmail based on form structure)
    const tcFirst = page.locator("#tcFirst");
    const tcLast = page.locator("#tcLast");
    if (await tcFirst.isVisible()) await tcFirst.fill("E2E TC");
    if (await tcLast.isVisible()) await tcLast.fill("Person");
    await page.locator("#tcEmail").fill(`tc-${uid}@example.com`);
